...
void IndexSettingsPanel::commit_to_config(WorkspaceConfig& cfg) const
{
    // Parse exclude patterns from multi-line text control.
    {
        ...
        cfg.exclude_patterns = std::move(patterns);
    }

    cfg.semantic_search_enabled = semantic_enabled_ctrl_->GetValue();
    cfg.embedding_model         = semantic_model_ctrl_->GetValue().ToStdString();
    cfg.reranker_enabled        = reranker_enabled_ctrl_->GetValue();
    cfg.reranker_model          = reranker_model_ctrl_->GetValue().ToStdString();
    cfg.reranker_top_k          = reranker_top_k_ctrl_->GetValue();
}
