...
wxDEFINE_EVENT(EVT_AGENT_UNVERIFIED_SUCCESS,  wxThreadEvent);
...
void WxFrontend::on_unverified_success(const std::string& note)
{
    auto* evt = new_evt(EVT_AGENT_UNVERIFIED_SUCCESS, tab_id_);
    evt->SetString(wxString::FromUTF8(note));
    wxQueueEvent(handler_, evt);
}
...
void WxFrontend::on_reasoning_watchdog_tripped(const std::string& trigger,
                                                int value)
{
    auto* evt = new_evt(EVT_AGENT_WATCHDOG_TRIPPED, tab_id_);
    evt->SetString(wxString::FromUTF8(trigger));
    evt->SetInt(value);
    wxQueueEvent(handler_, evt);
}
