...
- [ ] **[S6.12](S6.12-system-prompt-profiles.md)** -- System-Prompt Profiles (Full / Compact / Minimal via `system_prompt.profile` setting; trims the prose Rules / Editing / Shell / MSVC sections without losing load-bearing invariants; lazy-manifest-compatible)
- [ ] **[S6.13](S6.13-reasoning-watchdog.md)** -- Reasoning Watchdog + Commit-Now Button (per-round budget on reasoning seconds / chars / silent-rounds, OR semantics; non-modal "Commit now" button surfaces when tripped; agentic-mode `agent.reasoning_auto_nudge` setting fires the same action without user click)
