...
- [ ] **[S6.10](S6.10-small-model-robustness.md)** -- Small-Model Robustness Pass (10 tasks: **Task A** JSON repair pre-pass for tool-call bodies **done 2026-05-25** / quality monitor with 4-detector auto-correction / strip past-turn thinking from LLM payload / **Task D** server-side grammar-constrained decoding (response_format json_schema path; GBNF + server probe deferred) **done 2026-05-25** / few-shot examples in tool descriptions / auto-detect model + auto-apply preset / anti-truncation detection for code writes / per-model sampler defaults / **Task I** `ITool::short_description()` for lazy-manifest mode (S6.11 follow-up; ~half the remaining manifest budget) **done 2026-05-25** / **Task J** drop the prose `## Available Tools` section when `lazy + openai_json` (S6.11 follow-up; another ~800-1100 t off on openai-mode setups) **done 2026-05-25**)
...
- [ ] **[S6.17](S6.17-compaction-fixes-and-tool-robustness.md)** -- Compaction Fixes + Tool-Arg Robustness (TestLocalVibe2 follow-up: rebuild the compaction pipeline so layers escalate on `reached=no` and the sentinel preserves task progress; consolidate the five `layer_*` booleans into a `compaction.aggressiveness` preset; cross-tool `reject_unknown_keys` helper kills the silent-fallback class behind read_file's `lines:` / search's `type:` / run_command's `cmd:`; remove `filter_output` tool; split unified `SearchTool` into six per-mode tools per [ADR-0008](../../architecture/decisions/0008-split-unified-searchtool.md); `agent.prompt_cost` preset consolidates lazy_tool_manifest + system_prompt_profile + tool_format; reasoning-watchdog gets a 5s timer tick + commit-phase skip; index storage shrink -- lazy vec0 alloc + fix `chunk_code` nested-symbol bug that emits degenerate/duplicated chunks)
