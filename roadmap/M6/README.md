...
- [x] **[S6.13](S6.13-reasoning-watchdog.md)** -- Reasoning Watchdog + auto-nudge (per-round budget on reasoning seconds / chars, OR semantics; `agent.reasoning_auto_nudge` setting cancels the in-flight LLM stream and injects a "Stop reasoning, commit now" steering message; 2-nudge cap before turn aborts with "Agent appears stuck"; manual chat-panel button deferred to a follow-up) **done 2026-05-25**
