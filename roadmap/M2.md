...
## S2.2 -- Activity Details Panel done

- [x] `ActivityEvent` struct in `src/core/activity_event.h`: `id`, `timestamp`, `kind` enum (`system_prompt`, `user_message`, `llm_response`, `tool_call`, `tool_result`, `index_event`, `warning`, `error`), `summary` (one-line), `detail` (full text), optional `tokens_in`/`tokens_out`/`tokens_delta`
...
## S2.3 -- Document Text Extraction done
...
## S2.4 -- Attach File to Context done
...
Richer editor-driven context (line/col/selection) moves to **[S6.6](M6/S6.6-vscode-shim.md)** once the VS Code shim is feeding it.
