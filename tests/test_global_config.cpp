...
TEST_CASE("Missing global config returns compiled defaults", "[s5.m][global-config]")
{
    ...
    REQUIRE(cfg.llm.endpoint == "http://127.0.0.1:1234");
    REQUIRE(cfg.llm.max_tokens == 8192);
    REQUIRE(cfg.llm.tool_format == "auto");
    REQUIRE(cfg.index.respect_gitignore == true);
...
TEST_CASE("Save then load round-trips most fields", "[s5.m][global-config]")
{
    ...
    GlobalConfig src;
    src.llm.endpoint      = "http://127.0.0.1:8080";
    src.llm.model         = "gemma-4-e4b";
    src.llm.temperature   = 0.42;
    src.llm.max_tokens    = 16384;
    src.llm.tool_format   = "qwen";
    src.llm.top_p         = 0.8;
    src.llm.top_k         = 20;
    src.index.exclude_patterns  = {"foo/**", "*.bak"};
    src.git.auto_commit   = true;
    src.git.commit_prefix = "[bot] ";
    ...
    REQUIRE(loaded.llm.endpoint      == src.llm.endpoint);
    REQUIRE(loaded.llm.model         == src.llm.model);
    REQUIRE(loaded.llm.temperature   == src.llm.temperature);
    REQUIRE(loaded.llm.max_tokens    == src.llm.max_tokens);
    REQUIRE(loaded.llm.tool_format   == src.llm.tool_format);
    REQUIRE(loaded.llm.top_p         == src.llm.top_p);
    REQUIRE(loaded.llm.top_k         == src.llm.top_k);
    REQUIRE(loaded.index.exclude_patterns  == src.index.exclude_patterns);
    REQUIRE(loaded.git.auto_commit   == src.git.auto_commit);
    REQUIRE(loaded.git.commit_prefix == src.git.commit_prefix);
...
TEST_CASE("Workspace first-open seeds from global config", "[s5.m][global-config][workspace]")
{
    ...
    GlobalConfig src;
    src.llm.endpoint     = "http://127.0.0.1:9999";
    src.llm.max_tokens   = 24000;
    src.index.exclude_patterns = {"specifically_seeded/**"};
    REQUIRE(locus::save_global_config(src));
...

    {
        ...
        REQUIRE(cfg.llm.endpoint     == "http://127.0.0.1:9999");
        REQUIRE(cfg.llm.max_tokens   == 24000);
        REQUIRE(cfg.index.exclude_patterns == std::vector<std::string>{"specifically_seeded/**"});
    }
    ...
    changed_global.llm.endpoint = "http://different.host:1111";
