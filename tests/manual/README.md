...
| [workspace-trust.md](workspace-trust.md) | Workspace tool trust + outside-workspace warning + injection scan | Settings -> Tool Approvals per-tool Auto/Ask/Deny dropdowns, friction confirm on first Auto for mutating tools, run_command outside-workspace path detection re-triggers the approval panel with a yellow banner, and (S6.0) prompt-injection scan on untrusted MCP/ZIM ingress + the `security.injection_scan=false` off-switch. |
...
| [run-command-output-filter.md](run-command-output-filter.md) | run_command / read_process_output output filter | Settings spinner round-trip, default head_tail smart-truncate, explicit per-call `output_filter_mode` (head/tail/regex/substring) with pattern + context, raw recovery via `.locus/locus.log` at trace level. |
| [non-code-workspaces.md](non-code-workspaces.md) | Non-code workspace proof (S5.N) | End-to-end demo of WS2 (Kiwix-extracted Simple English Wikipedia) and WS3 (RFCs + DOCX/XLSX + Markdown notes): build scripts, capability defaults, 10 acceptance prompts, retrieval-eval smoke. |
| [prompt-cost.md](prompt-cost.md) | Lazy tool manifest + system-prompt profiles (S6.11 + S6.12) | Both Settings round-trip through `.locus/config.json` + persist across relaunch + invalid-string fallback. Token-cost log lines (`system prompt ~N tokens (profile=X)` + `Tool manifest: 12 tools, ~N tokens`) track each knob. `describe_tool('<name>')` fetches full schemas on demand when lazy mode is on, errors with closest-match suggestion on unknown names. |
| [reasoning-watchdog.md](reasoning-watchdog.md) | Reasoning watchdog + Commit-now button (S6.13) | Auto-nudge cancels the in-flight LLM stream + injects steering message when `reasoning_max_chars` / `reasoning_max_seconds` trips. Manual mode surfaces the `Commit now` chat-footer button instead -- click nudges, Stop aborts the turn. 2-nudge cap before "Agent appears stuck" abort. Needs a reasoning model loaded (Qwen 3.x with thinking on, DeepSeek-R1-distill). |
| [small-model-robustness.md](small-model-robustness.md) | Small-model robustness pass (S6.10) | Auto-detect preset on workspace open (Task F: `auto-applied preset 'X' for model 'Y'` log + Settings checkbox). "Reset samplers to preset" button (Task H: resets only the sampler block; preserves temperature). Truncation detector blocks code-elision markers (Task G: refusal message + activity event + off-switch + Python `Ellipsis` / JS spread negative case). Quality-monitor `repeated_tool_call` nudge (Task B: synthetic user message, shared 2-cap with the reasoning watchdog). |
| [llm-settings.md](llm-settings.md) | Thinking ON/OFF/auto knob (S6.14) | Settings -> LLM "Thinking" dropdown forces reasoning on/off at the source per model family (Qwen3 `enable_thinking`, Qwen2 `/no_think`, o1/R1 `reasoning_effort`). Off suppresses the Thoughts bubble on a Qwen 3.x model, On restores it, unknown family logs one warn + no-ops. Applied at workspace open (restart to take effect); startup log `thinking=` confirms. |
| [endpoints.md](endpoints.md) | Endpoint Profiles + chat-footer switcher (S6.16) | Settings > Endpoints add/edit/remove/set-active with masked key + "Show key" toggle, chat-footer endpoint picker + "Edit endpoints..." sentinel, mid-session hot-swap that preserves history and lands the next turn on the new endpoint, per-workspace override + `~/.locus/endpoints.json` persistence across restart, API key never written to `.locus/locus.log`. |
| [window-tray.md](window-tray.md) | Window minimize + Hide to Tray | Minimize stays plain taskbar behaviour (window never vanishes); View > Hide to Tray (Ctrl+Shift+H) hides into the notification area; tray double-click / Show Window restores to a normal non-minimized window. |
| [zim-reader.md](zim-reader.md) | ZIM reader (S6.2) | Open a Kiwix / Wikipedia `.zim` directly as a read-only workspace: index build + article count, search/read answers with the `[wikipedia, untrusted]` origin marker, read-only browsing, per-archive `.locus-zim-<name>` index reused on reopen. |
