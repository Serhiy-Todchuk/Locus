...
| `[s6.16]` | [test_int_endpoint_switch.cpp](test_int_endpoint_switch.cpp) | S6.16 -- endpoint hot-swap. Writes a temp global `endpoints.json` (via `LOCUS_GLOBAL_DIR`) with a local + NVIDIA Build profile, runs turn 1 on local, `request_endpoint_switch("NVIDIA Build (hosted)")`, asserts `active_base_url` / `active_endpoint_name` flip, runs turn 2 (a non-empty error-free response proves base_url + Bearer auth both landed -- NVIDIA 401s without a valid key), and that history survived the switch. **Reads the key from env `LOCUS_NVIDIA_API_KEY`; WARN-skips when unset so the repo carries no secret.** Needs LM Studio for the harness's local turn. |
