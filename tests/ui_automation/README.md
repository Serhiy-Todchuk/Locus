...
| `select_tab`           | `parent_aid` + `index` (zero-based), or `name` for label-bearing tabs | wxNotebook on Windows wraps SysTabControl32 which doesn't propagate tab labels to UIA Name -- so `parent_aid` + `index` is the reliable shape. The `name` shape works for wxAuiNotebook and other strips that DO surface labels. **Lazy realize:** wxNotebook only realizes the ACTIVE page's child widgets; non-active pages live in the tree as bare `TabItem` placeholders with no addressable children. If a script needs to `find` a widget that lives on a non-active page, `select_tab` to it FIRST. (Example: `terminal_stdin.json` waits for the bg process to spawn, selects the bg sub-tab, THEN finds the `stdin_input` widget -- finding before selecting always times out.) |
...
| [`security_settings.json`](scripts/security_settings.json)         | no  | S6.0 -- Settings -> Security tab (index 7). Asserts the four scanner widgets are reachable (`locus.settings.security.{injection_scan,scan_zim,block_confidence,max_scan_kb}`), toggles `injection_scan` off + sets `max_scan_kb=129`, OK, then `assert_file_contains` on `.locus/config.json` confirms both edits round-tripped (file assertion is the reliable signal -- reading checkbox state back via UIA is flaky). |
| [`web_settings.json`](scripts/web_settings.json)                   | no  | S6.1 -- Settings -> Web tab (index 8). Asserts the nine web-retrieval widgets are reachable (`locus.settings.web.{enabled,provider,api_key,api_url,max_results,cache_ttl_hours,cache_max_mb,max_web_page_kb,allow_http}`), enables web + sets a unique `api_url` marker + `cache_max_mb=77`, OK, then `assert_file_contains` on `.locus/config.json` confirms the web section round-tripped. The provider `wxChoice` is only asserted reachable, not changed (CBS_DROPDOWNLIST selection-by-key is unreliable under UIA). |
