...
| `agent.lazy_tool_manifest` **(S6.11)** | true | Collapses the per-turn tool catalog (both system-prompt `## Available Tools` and the API `tools: [...]` array) to one-line summaries; the model fetches the full schema via the `describe_tool('<name>')` meta-tool. Saves ~3.6K tokens per turn on the default capability matrix. Default flipped to true on 2026-05-28 -- the canonical context-budget win for any local LLM, and the arg-shape error path now injects the canonical schema on every failure so the "extra round-trip on first miss" cost is absorbed automatically. Set to `false` to ship full schemas every turn (useful for very-large-context cloud models where context isn't the bottleneck). |
