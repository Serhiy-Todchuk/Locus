...
**M3 — Refactoring in progress.** S3.C (`IWorkspaceServices`), S3.F (`WatcherPump` + `LocusFrame` split), S3.A (`AgentCore` split into `AgentLoop` / `ToolDispatcher` / `ActivityLog` / `ContextBudget`; agent sources moved to `src/agent/`), S3.J (`SlashCommandParser` + `SlashCommandDispatcher` extracted; GUI popup sources completions from the dispatcher), S3.E (`tools.cpp` split into `src/tools/` by family — shared / file / search / index / process / interactive; `tools.h` is now an aggregator re-export), S3.K (`architecture/agent-loop.md` updated for post-S3.A split, `architecture/decisions/` ADR trail, `overview.md` see-also footer), S3.I (`architecture/threading-model.md` + `ConversationHistory::assert_owner_thread()` fence around every mutating entry point; agent thread sets ownership per turn via `ConversationOwnerScope` RAII) and S3.L (tool-catalog hygiene: four `search_*` tools collapsed into one `search(mode=…)` face; `ITool::available()` / `visible_in_mode()` gating predicates; `ToolRegistry::build_schema_json(ws, mode)` filtered overload; `AgentLoop` logs the per-turn manifest token footprint and warns when it crosses `WorkspaceConfig::tool_manifest_warn_tokens` — default 4000) done. See [roadmap/M3/](roadmap/M3/) for the task list.
