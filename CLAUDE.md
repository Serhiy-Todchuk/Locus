...
| `src/llm/llm_client.h/cpp` | Public `ILLMClient` interface + types (`ChatMessage`, `ToolCallRequest`, `LLMConfig`, `StreamCallbacks`, `CompletionUsage`, `ToolSchema`, `ModelInfo`). The .cpp also holds the concrete `LMStudioClient` (composes `OpenAiTransport` + `OpenAiDecoder`) and the `create_llm_client` factory. Post-S3.B: no `estimate_tokens` static -- see `TokenCounter`. | `ILLMClient`, `LMStudioClient`, `ChatMessage`, `LLMConfig`, `StreamCallbacks` |
...
| `src/llm/stream_decoder.h` | `IStreamDecoder` interface -- translates one SSE payload into typed events (`on_text` / `on_reasoning` / `on_tool_call_delta` / `on_usage`) via a stack-allocated `StreamDecoderSink`. Decoders may keep per-stream state (XML formats need it); stateless decoders ignore `reset()`. | `IStreamDecoder`, `StreamDecoderSink` |
...
| `src/llm/stream_decoders/qwen_xml_decoder.h/cpp` | S4.N stub. `decode()` throws `std::logic_error` so the router can't accidentally select it before the format is implemented. | `QwenXmlDecoder` |
| `src/llm/stream_decoders/claude_xml_decoder.h/cpp` | S4.N stub. Same shape as `QwenXmlDecoder` -- header + throwing impl. | `ClaudeXmlDecoder` |
