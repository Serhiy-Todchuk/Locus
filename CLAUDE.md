...
| `src/core/workspace.h/cpp` | Opens a folder, owns main_db (always) + vectors_db (optional) + watcher + indexer + query + watcher_pump. One per folder. `WorkspaceConfig` is the single source of truth for per-workspace knobs (see the header for the full list -- recent additions: `tool_max_runtime_s`, `dump_on_run_command_hang`, `max_rounds_per_message` from the agentic Tetris follow-ups). | `Workspace`, `WorkspaceConfig` |
