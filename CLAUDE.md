...
**M6 -- Connected & Misc -- in progress.** Done: S6.9 (Stage A complete -- macOS arm64 build + CLI + GUI chat surface + agent + retrieval green; Stage B shell/MCP POSIX pending, Stage C deferred), S6.10, S6.11, S6.12, S6.13, S6.15, S6.16, S6.17, S6.18 (partial -- Tasks A-G shipped, remaining items deferred), S6.19, S6.20. Per-stage detail in [roadmap/M6/](roadmap/M6/); cross-cutting design rationale in [ADR-0007](architecture/decisions/0007-context-budget-reshape-lazy-manifest-and-profiles.md).
...
Note: M3 = Refactoring (not Agent Quality) due to renumbering -- see [roadmap.md](roadmap.md) for the full history.
...
- **Lazy vec0 tables + `chunks.id` rowid-reuse safety** (S6.18 Task A). The vec0 tables `chunk_vectors` and `memory_vectors` are NOT created eagerly by `Database::ensure_vectors_schema`; writers call `ensure_chunk_vectors_table()` / `ensure_memory_vectors_table()` before their first INSERT. Read paths gate on `chunk_vectors_present()` / `memory_vectors_present()` and the prepared statements that reference these tables (`IndexerStatements::delete_chunk_vecs`, `IndexQuery::stmt_search_semantic_`, `MemoryStore::stmt_vec_*`) are prepared lazily on first use after the table has been minted. Adding a new tool / subsystem that touches either vec0 table MUST call the matching `ensure_*_table()` before any INSERT and MUST gate any SELECT/DELETE on `*_present()` -- otherwise prepare will fail on brand-new workspaces. Sibling rule: `chunks.id` is `INTEGER PRIMARY KEY` without `AUTOINCREMENT`, so SQLite recycles freed rowids after `DELETE FROM chunks WHERE file_id=N`. A recycled chunk_id can land on a `chunk_vectors` row that still holds the freed predecessor's embedding. **`INSERT OR REPLACE` does NOT fix this on a vec0 virtual table** -- sqlite-vec's `xUpdate` ignores the REPLACE conflict action, so the insert raises `SQLITE_CONSTRAINT` ("UNIQUE constraint failed on chunk_vectors primary key") and the *stale* vector survives (silently wrong, not just noisy). The reliable pattern (2026-05-29 fix) is an explicit **`DELETE FROM chunk_vectors WHERE chunk_id=?` immediately before a plain `INSERT`** -- see `EmbeddingWorker::thread_func` (delete-then-insert) and `MemoryStore::vec_upsert` (deletes the colliding `memory_rowid` before insert). Any new writer of either vec0 table MUST follow delete-then-insert, not rely on OR REPLACE. Switching `chunks.id` to AUTOINCREMENT would require a schema migration + a chunk_vectors rowid remap; strictly worse than the delete-then-insert pattern.
...
- **Tool-arg reads on the model path must not throw; `preview()` has a dispatcher backstop** (S6.19). A model that emits a tool arg with the WRONG JSON type -- a string where a bool/array/int is expected (`"overwrite":"True"`, `"edits":"[...]"`, `"offset":"2"`) -- is routine for small / hosted models. `nlohmann::json::value<T>(key, default)` and `.get<T>()` THROW `type_error` on a type mismatch; when that throw lands in `ITool::preview()` (which runs on the agent thread during the `on_tool_call_pending` broadcast, even for auto-approved tools) there was no catch and the process fail-fasted (`0xc0000409` -- a single mistyped token from the model killed the GUI). Rule: every LLM-reachable tool-arg read goes through one of the never-throwing coercers in [src/tools/shared.{h,cpp}](src/tools/shared.h) -- `coerce_bool` / `coerce_int` / `coerce_json_array` (each returns the caller's fallback on an unusable type; `coerce_json_array` also unwraps a JSON-string-encoded array so the model's `"edits":"[...]"` slip *works* instead of erroring) -- OR is `.is_*()`-guarded before `.get<T>()`. Independently, `tool->preview(call)` in [ToolDispatcher](src/agent/tool_dispatcher.cpp) is wrapped in a try/catch (`std::exception` + `...`) that logs a warn and substitutes a fallback preview string -- the backstop that makes the whole crash class unreachable regardless of which arg/tool a future change misses. Do NOT reintroduce a raw `call.args.value<bool/int>(...)` / `e.value("edits", json::array())` on a model-supplied object. Tests: [tests/test_tool_arg_validation.cpp](tests/test_tool_arg_validation.cpp) tags `[coerce_bool]` / `[coerce_int]` / `[coerce_json_array]`.
- **Transient transport failures retry via the pure `classify_retry`; deterministic ones do not** (S6.19). Free hosted tiers (NVIDIA Build, OpenRouter free) intermittently return HTTP 429, HTTP 5xx, or a clean HTTP 200 with an EMPTY body / zero SSE chunks after a long queue wait. Before S6.19 only `cpr OPERATION_TIMEDOUT` retried (once); 429 / 5xx / empty-200 fell straight to `on_error` and aborted the whole agent turn, discarding all progress. The retry decision is the single pure function `classify_retry(cpr_error, http_status, chunks, cancelled, attempt, max_attempts, retry_after_ms, ...)` in [src/llm/openai_transport.{h,cpp}](src/llm/openai_transport.h): retryable = timeout / 429 / 5xx / 200-with-zero-chunks; NOT retryable = connect-refused, 4xx other than 429 (bad request / auth), or any cancelled call. `OpenAiTransport::do_post` is a bounded loop (`k_max_attempts = 5`) that only does side effects (exponential backoff sleep, re-check the cancel flag across the backoff, recurse with `attempt+1`); any new retry rule belongs in `classify_retry`, not the loop. Backoff is exponential off a base, capped, floor raised to `Retry-After`; **429 gets a higher floor (8s)** because rate-limit windows outlast a 2s/4s backoff. Tests: [tests/test_endpoint_profile.cpp](tests/test_endpoint_profile.cpp) tag `[retry]`. Sibling rule still holds: the empty-200 terminal case (all retries exhausted) is surfaced by `LMStudioClient` via the `got_data==false` check, not by the transport.
- **Build-error "stuck" detection is a pure function; the streak/nudge/abort policy lives on `AgentTurnRunner`** (S6.20). A model thrashing on a large file emits a DIFFERENT tool call each round (a fresh rewrite) but the SAME compiler error -- so the repeated-tool-call QualityMonitor (which keys on identical (name, args)) never fires. `extract_error_signature(tool_output)` in [src/agent/build_error_detector.{h,cpp}](src/agent/build_error_detector.h) is the pure detector: it normalizes compiler/linker diagnostics to a signature that is invariant to line/column numbers, paths, hex addresses, counts ("3 unresolved externals"), and diagnostic order (sorted + de-duped) WHILE preserving error codes (so `C2102` != `C2065`). Any new diagnostic shape belongs in this function (MSVC `error C####` / `fatal error LNK####` + GCC/Clang `path:line:col: error:` are covered; warnings are intentionally ignored). The streak policy lives on `AgentTurnRunner` (per-turn state): aggregate each round's tool output, bump a consecutive-identical-signature counter, nudge once via `inject_nudge` on the 3rd repeat (`k_stuck_error_streak`), abort the turn if it recurs after the nudge; a no-error round resets the streak. Sibling rule (S6.20 #2): `AgentTurnRunner::note_write_call` flags a large (>= ~16 KB) `write_file overwrite`, and `maybe_hint_large_overwrite` injects a one-time-per-turn edit_file hint when a build error follows -- both gated by `agent.quality_monitor_enabled` and sharing the 2-nudge cap. Tests: [tests/test_build_error_detector.cpp](tests/test_build_error_detector.cpp) tag `[build_error_detector]`.
- **Endpoint base_url joins through `join_api_url`, never raw `base_url + "/v1/..."`** (S6.16). Hosted providers (NVIDIA / Ollama / OpenRouter) document their base_url WITH the `/v1` suffix; LM Studio's is host-only. `join_api_url(base, "/v1/chat/completions")` in [llm_client.h](src/llm/llm_client.h) trims the duplicate `/v1` when the base already ends in it -- without it the URL doubles to `.../v1/v1/chat/completions` and 404s on every hosted endpoint. Both [openai_transport.cpp](src/llm/openai_transport.cpp) (`/v1/chat/completions`) and `LMStudioClient::query_model_info` (`/v1/models`) MUST route through it; a refactor that reverts to string concatenation silently breaks every `/v1`-suffixed profile. Sibling rule: the API key flows into `Authorization: Bearer <key>` via `build_request_headers` and is NEVER logged -- the only transport trace prints the URL, not the header block. Endpoint hot-swap reseats `AgentLoop::llm_` (a pointer) via `set_llm` on the agent thread between turns; `AgentCore::owned_llm_` owns the swapped client and `active_llm()` is the single read path (compaction + loop both use it, never the ctor `llm_` reference directly).
- **macOS build: arm64-only, single-config Ninja, Metal embedder, POSIX-stub process tools** (S6.9 Stage A). vcpkg `ggml[metal]` (osx-gated in [vcpkg.json](vcpkg.json)) runs the in-process bge/reranker on the GPU; without it the embedder falls back to CPU and the integration harness -- which indexes the WHOLE repo -- stalls ~20 min at setup instead of ~315 ms. PDFium ships as `libpdfium.dylib`: per-platform path on the `PDFIUM_RUNTIME_LIB` cache var (NOT the legacy `PDFIUM_DLL`; the three test CMakeLists were migrated too), install name rewritten to `@rpath` + ad-hoc re-signed via `execute_process` at configure, `locus`/`locus_gui` carry `BUILD_RPATH=@executable_path`. The S5.P em-dash lint resolves its interpreter via `find_program(NAMES python3 python)` (modern macOS has no bare `python`). libc++ is stricter than MSVC: needs explicit `<thread>`/`<chrono>`, no `std::chrono::clock_cast` (use the file_clock->system_clock offset idiom), no NSDMI-in-default-arg, no `long`->`int32` brace-narrowing. `run_command` / `run_command_bg` / MCP stdio stay `[platform: macos]` stubs until Stage B. Build steps: [CONTRIBUTING.md](CONTRIBUTING.md) section "Building on macOS".
- **Chat WebView readiness is signalled by the page, not by `wxEVT_WEBVIEW_LOADED`** (S6.9 Stage A GUI). `ChatPanel` queues every `RunScript` until `page_ready_` flips, then flushes. `wxEVT_WEBVIEW_LOADED` fires AFTER the inline document script on WebView2 (Windows) but BEFORE it on WKWebView (macOS) -- so trusting LOADED alone calls undefined JS functions on macOS (`ReferenceError: Can't find variable: addSystemPromptMsg`). The contract: the LAST `<script>` block in [chat_html_template.cpp](src/frontends/gui/chat_html_template.cpp), after every function is defined, navigates to `locus://ready`; [ChatPanel::on_webview_navigating](src/frontends/gui/chat_panel.cpp) intercepts that URL BEFORE the `page_ready_` guard and calls the idempotent `ChatPanel::mark_page_ready()` (the sole flush path -- the LOADED handler also calls it, whichever lands first wins). Don't move the `locus://ready` emit earlier in the document, don't drop it, and don't reorder the `on_webview_navigating` interception after the `if (!page_ready_) return;` guard -- any of the three silently re-breaks the macOS chat surface while leaving Windows green.
...
#        [s6.17][prompt_cost] [s6.17][compaction-replay] [s6.16])
```
...
| `src/llm/llm_client.h/cpp` | Public `ILLMClient` interface + types (`ChatMessage`, `ToolCallRequest`, `LLMConfig`, `StreamCallbacks`, `ToolFormat`, `GrammarMode`). `LLMConfig` carries S6.16 `api_key` + `extra_headers`. The .cpp holds the concrete `LMStudioClient` (composes `OpenAiTransport` + a per-`ToolFormat` `IStreamDecoder`), the `create_llm_client` factory, and the BestEffort grammar-retry intercept that drops `response_format` once and re-streams when the server rejects it. `join_api_url(base, path)` (inline) joins an OpenAI-compat base_url with a `/v1/...` path WITHOUT doubling `/v1` when the base already ends in it (NVIDIA / Ollama / OpenRouter document their base with the suffix); the transport + `query_model_info` both route through it. | `ILLMClient`, `LMStudioClient`, `ChatMessage`, `LLMConfig`, `ToolFormat`, `GrammarMode`, `join_api_url` |
| `src/llm/endpoint_profile.h/cpp` | S6.16 -- `EndpointProfile` (one OpenAI-compat LLM source: name / base_url / api_key / default_model / default_context_limit / tool_format / extra_headers / builtin) + `EndpointProfileStore` (load/save/find/add/update/remove/set_active against the global `~/.locus/endpoints.json` via `endpoints_path()`; seeds six builtins -- LM Studio / Ollama / NVIDIA Build / OpenAI / OpenRouter / Claude-via-proxy -- when the file is absent; builtins are editable-not-removable). `apply_endpoint_profile(cfg, profile, force)` is the single layering helper: `force=false` (startup resolution) fills only unset/default sentinels so a non-default `.locus/config.json llm.endpoint` or caller seed wins; `force=true` (hot-swap) overlays endpoint identity unconditionally. `api_key`/`extra_headers` always come from the profile. | `EndpointProfile`, `EndpointProfileStore`, `endpoints_path`, `apply_endpoint_profile` |
| `src/llm/openai_transport.h/cpp` | cpr POST `/v1/chat/completions` + SSE feed + bounded transient-failure retry + HTTP error reporting. Hands each `data:` payload to its callback as a raw string and swallows `[DONE]`. Knows nothing about JSON. `build_request_headers(config)` (free fn, unit-tested) builds the header list: always Content-Type + Accept, `Authorization: Bearer <api_key>` when the S6.16 key is set, then `extra_headers` merged verbatim -- the key is never logged at any level. URL via `join_api_url` (no `/v1` doubling). S6.19: `classify_retry(...)` (pure, unit-tested) decides whether one attempt's outcome is transient-retryable (timeout / 429 / 5xx / empty-200) with exponential backoff (8s floor for 429, `Retry-After` honoured); `do_post(body, cbs, attempt)` is a bounded retry loop (`k_max_attempts`) that only does the side effects (sleep, cancel re-check, recurse) -- see the Key Invariant "Transient transport failures retry via the pure classify_retry". Emits an info-level chunk heartbeat every 32 chunks OR every 10 s (Key Invariant -- "Info-level LLM stream heartbeat"; closes TestLocalVibe finding #1). Also emits `first chunk after X ms` (info), `inter-chunk gap X ms` (warn at >60s), and an end-of-stream summary. | `OpenAiTransport`, `build_request_headers`, `classify_retry`, `RetryDecision` |
| `src/llm/sse_parser.h/cpp` | Low-level SSE `data:` line parser. Used by `OpenAiTransport`. | `SseParser` |
| `src/llm/json_repair.h/cpp` | JSON repair pre-pass for malformed tool-call bodies. `repair_for_parse(input)` runs six stages (trailing commas, unquoted keys, single-quoted strings, raw control chars, brace balancing, fallback block-extract). Called from `XmlToolCallExtractor::parse_qwen_body` and `ToolRegistry::parse_tool_call` on parse failure. Pure C++. | `repair_for_parse`, `JsonRepairResult` |
| `src/llm/tool_call_grammar.h/cpp` | Server-side grammar-constrained decoding helpers. `build_tool_call_union_schema(tools)` produces the OpenAI Structured Output `response_format.json_schema` (oneOf per-tool variants). `grammar_should_attach(mode, fmt, tool_count)` is the gate. Consumed by `LMStudioClient::build_request_body`. | `build_tool_call_union_schema`, `grammar_should_attach` |
...
| `src/llm/model_presets.h/cpp` | Static known-good defaults per LLM backend + model family (Gemma, Qwen, Qwen3-Coder, DeepSeek-R1-Distill, Llama 3.x, Ollama / llama-server generic). `ModelPreset` carries base_url / temperature / max_tokens / tool_format / sampler / grammar_mode. `find_preset_for_model(model_id)` is a regex matcher consumed by `LocusSession::load_shared_resources` when `auto_detect_preset` is on. | `ModelPreset`, `builtin_presets`, `find_preset`, `find_preset_for_model` |
...
| `src/core/memory_store.h/cpp` | Workspace-scoped memory bank. Owns `.locus/memory/<id>.md` (verbatim entries with YAML frontmatter) + `memory_fts` (main_db) + `memory_vectors` vec0 (vectors_db) with `memory_keys(rowid, id)` text-id mapping. Add / update / soft_delete (`.deleted/` retention) / hard_delete / restore_deleted / purge_deleted / gc. Hybrid `search` blends BM25 + cosine + 21-day recency + optional reranker; raw queries are phrase-wrapped before FTS5 binding so punctuation doesn't parse as operators. `format_for_system_prompt()` composes the "## Memory Bank" slot. Owned by `Workspace`; exposed via `IWorkspaceServices::memory()`. Reconciles FTS5 + vector rows against on-disk `.md` on construction so a deleted vectors.db / model swap re-embeds cleanly. | `MemoryStore`, `MemoryStore::Entry`, `MemoryStore::SearchHit` |
...
| `src/tools/shared.h/cpp` | Shared helpers for tool implementations in namespace `locus::tools`: `resolve_path` + `is_inside` (component-aware workspace containment), `write_atomic` (temp file + rename with cross-volume fallback), `reject_unknown_keys` (with optional schema injection for self-healing under lazy manifest), `render_tool_schema` / `error_with_schema` / `missing_required_arg`, and the S6.19 never-throwing tool-arg coercers `coerce_bool` / `coerce_int` / `coerce_json_array` (read a model-supplied arg of the wrong JSON type without throwing -- see the Key Invariant "Tool-arg reads on the model path must not throw"). | `is_inside`, `write_atomic`, `resolve_path`, `render_tool_schema`, `error_with_schema`, `missing_required_arg`, `coerce_bool`, `coerce_int`, `coerce_json_array` |
| `src/tools/command_path_scanner.h/cpp` | S4.V Task 5 pure scanner. `scan_outside_workspace_paths(cmd, root)` tokenises on whitespace (single/double-quoted runs become one token, quotes stripped) and flags absolute Windows / UNC / Unix paths, parent-traversal segments, and env-var or `~` expansions whose canonical form lies outside `root`. `ToolDispatcher` runs it for `run_command` / `run_command_bg`; non-empty results downgrade `auto_approve` -> `ask` and flow as `safety_warnings` through `IFrontend::on_tool_call_pending` -> approval panel banner. Doesn't apply to file tools (they already refuse outside-workspace paths via `resolve_path`). S6.9: parent-traversal resolution normalizes backslashes to `/` first, so a Windows-style `..\..\x` is flagged on every host (off-Windows `fs::path` treats `\` as an ordinary char, which previously let it slip through). | `scan_outside_workspace_paths` |
| `src/tools/permission_presets.h/cpp` | Named permission preset matrix (`read_only` / `ask_before_edits` / `allow_edits` / `allow_all` / `custom`). `builtin_tool_category(name)` maps tools to {read, edit, delete, memory, process, shell, interactive, plan, mcp, other} so new tools slot in by category. `preset_signature` / `detect_preset` round-trip between preset and per-tool override map. **Read tools are unconditionally auto-approved** -- enforced both here and at dispatch in `ToolDispatcher`. | `PermissionPreset`, `builtin_tool_category`, `policy_for_category`, `preset_signature`, `detect_preset` |
| `src/tools/file_tools.h/cpp` | File CRUD + exact-string-diff edit tool. `EditFileTool` canonical args are `file_path` + `edits_array` (legacy `path` / `edits` still accepted -- canonical wins on conflict). `WriteFileTool` subsumes the old `CreateFileTool` via an `overwrite` flag (default false). `ReadFileTool` branches on extension: PDF / DOCX / XLSX read paginated extractor text from `files_fts.content`; code / Markdown / HTML / text go through raw bytes. Both paths share `render_line_window` and fire `mark_read` so `edit_file`'s precondition is satisfied uniformly. | `ReadFileTool`, `WriteFileTool`, `EditFileTool`, `DeleteFileTool` |
| `src/tools/search_tools.h/cpp` | Five per-mode top-level search tools (`search_text` / `search_regex` / `search_symbols` / `search_semantic` / `search_ast`) -- the unified `SearchTool` dispatcher and `search_hybrid` are retired (see [ADR-0008](architecture/decisions/) + [ADR-0009](architecture/decisions/)). Capability gating via per-tool `available()`: semantic gates on `capabilities.semantic_search`, symbols/ast on `capabilities.code_aware_search`. Each `execute()` opens with `tools::reject_unknown_keys`. `max_results` defaults: text 8, semantic 5, regex/ast/symbols 50. `split_path_glob()` accepts `,` / `|`-separated patterns and collapses trivial match-all shapes to "no filter". `IndexQuery::search_hybrid` stays callable from the retrieval-eval harness. | `SearchTextTool`, `SearchRegexTool`, `SearchSymbolsTool`, `SearchSemanticTool`, `SearchAstTool` |
| `src/tools/index_tools.h/cpp` | Listing + outline tools. `ListDirectoryTool` enumerates the real filesystem and uses `IndexQuery::list_directory` only for annotation -- per-entry tags: `[<lang>]` / `[binary]` / `[oversized]` / `[unindexed]`. Workspace `exclude_patterns` enforced at the tool layer. Output capped at `max_entries` (default 200). Case-insensitive sibling lookup catches Windows case-divergence. `GetFileOutlineTool` branches on filesystem reality when the index returns zero entries (non-existent / oversized / no-symbols-on-disk) with actionable next-tool hints. | `ListDirectoryTool`, `GetFileOutlineTool` |
| `src/tools/process_tools.h/cpp` | Sync shell execution + background-process tools (`run_command_bg`, `read_process_output`, `stop_process`, `list_processes`). Bg tools gate on `ProcessRegistry` presence. Default sync timeout 30 min (hidden from schema -- LLMs underestimate). Output trim via flat `output_filter_*` params delegating to [process_output_filter](src/tools/process_output_filter.h); raw-output backstop 1 MB. Full pre-filter body trace-logged. | `RunCommandTool`, `RunCommandBgTool`, `ReadProcessOutputTool`, `StopProcessTool`, `ListProcessesTool` |
| `src/tools/process_output_filter.h/cpp` | Shared output-trimming helper for `run_command` / `read_process_output`. `OutputFilterSpec` (mode: head_tail / head / tail / regex / substring), `apply_output_filter` returns the trimmed body with an elision marker pointing at the full log. Pure CPU, no workspace deps. | `OutputFilterSpec`, `apply_output_filter`, `parse_output_filter_args` |
| `src/tools/process_registry.h/cpp` | Per-workspace registry of long-running shell processes. Each child runs under a Win32 Job Object (`KILL_ON_JOB_CLOSE`); per-process reader thread drains stdout+stderr into a ring buffer. Owned by `Workspace`; dtor terminates every still-running child. `ProcessSinkBroker` fans chunks + lifecycle out to the terminal panel. `write_stdin(id, data)` forwards keystrokes via a mutex-guarded parent-side pipe. | `ProcessRegistry`, `BackgroundProcess` |
| `src/tools/process_sink.h` | Header-only `IProcessSink` interface + thread-safe `ProcessSinkBroker` with multi-subscriber `add_sink` / `remove_sink`. Six methods: `on_{bg,sync}_{started,chunk,exited}`. Called from worker threads; subscribers must queue chunks and not call wx widgets directly. Subscribers must NOT call add/remove inside an emit callback (deadlocks on the broker mutex). | `IProcessSink`, `ProcessSinkBroker` |
...
| `src/mcp/mcp_manager.h/cpp` | Per-workspace MCP pool. `start_all()` reads merged `mcp.json`, spawns + initialises each enabled server, and registers each tool as `mcp:<server>:<tool>`. Failed servers stay in the manager but contribute no tools. `restart(name)` hot-reloads one server. `status_snapshot()` powers the Settings panel. Lifetime: declared between `llm_` and `tools_` in `LocusSession` so the McpTool entries destruct before the McpClient instances they borrow (see [Workspace teardown order](#key-invariants)). | `McpManager`, `McpManager::ServerStatus` |
...
| `src/tools/truncation_detector.h/cpp` | Anti-truncation detector for code writes. `detect_truncation_phrase(content)` scans the last 200 chars against a curated phrase list (`// rest of the code`, `// ... existing code ...`, etc.). Called by `WriteFileTool` / `EditFileTool` before `write_atomic`; detection returns `ToolResult` with `activity_tag="truncation_blocked"`. Bare `...` is NOT in the list (collides with Python / JS / C++ varargs). | `detect_truncation_phrase` |
...
| `src/core/frontend.h` | Frontend/core interfaces (pure abstract + enums). Notable hooks: `on_history_message_added/deleted` (per-message-delete UIs), `on_round_progress(round, max_rounds)` (in-flight chip), `on_compaction_archived(counter, no_op_count=0)`, `on_reasoning_watchdog_tripped/cleared`. `ILocusCore::delete_message(int id)` + `request_commit_now()` are the chat-side dispatch entry points. | `IFrontend`, `ILocusCore`, `ToolDecision`, `CompactionStrategy` |
...
| `src/agent/system_prompt_assembly.h/cpp` | Immutable system-prompt type -- only `build()` factory constructs it; readers are `const`. Type-level enforcement of the byte-stable system prompt invariant (see Key invariants). Exposes per-section token counts + `total_tokens()` (exact-by-construction) + `hash()` (debug canary). `build()` takes `lazy_manifest` (collapses `## Available Tools` to per-tool one-liners or to a `## Tools` API-array pointer for openai_json / auto formats) and `SystemPromptProfile { Full, Compact, Minimal }` (picks one of three statically-rendered Rules / Editing / Shell / MSVC bodies). Workspace metadata / LOCUS.md / memory / tools section / format addendum are profile-invariant. | `SystemPromptAssembly`, `SystemPromptProfile` |
...
| `src/agent/tool_dispatcher.h/cpp` | Tool approval gate + execute + result injection. Resolves approval policy, waits on decision condvar (`call_id`-keyed `pending_decisions_`), writes the tool-result through an `AppendFn` so `AgentCore` stays sole `ConversationHistory` writer. `set_turn_context()` arms a per-turn `CheckpointStore` for `edit_file` / `write_file` / `delete_file` snapshots. `execute()` is wrapped in catch-all so tool exceptions surface as `ToolResult{success=false}` not agent-thread unwind. After a successful edit/delete tool, calls `watcher_pump().flush_now()` so the agent's own mid-turn writes are visible to its own subsequent reads. | `ToolDispatcher` |
| `src/agent/agent_loop.h/cpp` | One LLM round trip: builds the tool schema, streams tokens/reasoning/tool-call fragments, emits `llm_response` activity, returns `AgentStepResult`. No knowledge of tool execution. Threads `lazy_tool_manifest` into `ToolRegistry::build_schema_json(..., lazy)`. Reasoning watchdog: pure `evaluate_watchdog(WatchdogDecisionInputs)` decides chars/seconds trip; the closure handles side effects (CAS-to-single-trip, warn log, frontend broadcast, cancel flag). 5s timer thread maintains `recent_progress` for the productive-reasoning skip; chars and seconds are evaluated independently. See [Watchdog decision invariant](#key-invariants). | `AgentLoop`, `AgentStepResult`, `WatchdogDecisionInputs`, `evaluate_watchdog` |
| `src/agent/slash_commands.h/cpp` | Slash-command tokenizer + dispatcher. Pure parser (quoted args, key=value, positional, typed coercion); `ParsedSlashCall` carries `positional` + `kwargs` for prompt-template substitution alongside the typed `args` JSON. Dispatcher owns `/help`, `complete(prefix)` for autocomplete (`kind = "tool" \| "template"`), and routes tool-name slashes (`/read_file`) to `ITool::execute`. Built-in slashes intercepted earlier in `AgentCore::try_slash_command`: `/undo`, `/metrics`, `/export_metrics`, `/breakdown`, `/compact`, `/reload`, `/forget`, `/memorize`. `/save` is menu + Ctrl+S only. | `SlashCommandParser`, `SlashCommandDispatcher`, `ParsedSlashCall`, `SlashCompletion`, `SlashParseError` |
...
| `src/agent/quality_monitor.h/cpp` | Stateless detector over `ConversationHistory`. `evaluate(messages)` returns at most one `QualityCorrection` of kind `empty_response` / `repeated_tool_call` (arg JSON canonicalised for order-insensitive equality). Consumed by `AgentCore::inject_nudge` (shared 2-cap nudge counter with the watchdog). Each fired correction emits an `ActivityKind::quality_correction` event. `malformed_args` / `unknown_tool` are handled by JSON repair + dispatcher closest-match, not here. The S6.20 stuck-on-same-BUILD-error case (different calls, same compiler error) is NOT here -- it needs per-round state, so it lives on `AgentTurnRunner` + `build_error_detector`. | `QualityMonitor`, `QualityCorrection`, `QualityCorrectionKind` |
| `src/agent/build_error_detector.h/cpp` | S6.20 pure helper. `extract_error_signature(tool_output)` normalizes compiler/linker diagnostics (MSVC `error C####` / `fatal error LNK####`, GCC/Clang `path:line:col: error:`) into a stable signature -- strips line/col/path/hex/count noise + sorts + de-dupes, but preserves error codes -- so two builds that fail the same way (even after a rewrite moves the error to a new line) compare equal. Returns empty for success / warnings / non-build output. Consumed by `AgentTurnRunner::check_stuck_error` to detect a model looping on the same error. Dependency-free; unit-tested without a turn. | `extract_error_signature` |
| `src/agent/agent_core.h/cpp` | Orchestrator: agent thread + message queue + lifecycle + plan-mode state. Composes `LLMContext` (LLM-view state) alongside `AgentLoop` / `ToolDispatcher` / `ActivityLog` / `MetricsAggregator` / `SlashCommandDispatcher` / `PromptTemplateRegistry`. Owns `SessionManager`. Conversation-state methods (`history()`, `attached_context()`, ...) are passthroughs into `ctx_`. `process_message` is thin: setup + `AgentTurnRunner(...).run()` + post-loop bookkeeping. `request_commit_now()` sets nudge + cancel flags together; `nudges_this_turn_` is hard-capped at 2 before the turn aborts. | `AgentCore` |
| `src/agent/agent_turn.h/cpp` | `AgentTurnRunner` -- multi-round LLM/tool loop for one user-message turn. Stack-allocated by `process_message`; runs on the agent thread. `friend class AgentTurnRunner` in `agent_core.h` grants access to AgentCore's private atomics / subsystem members. Owns the auto-compact band check, mid-turn user-message drain, cancel/nudge fork (Stop vs commit-now vs auto-nudge), QualityMonitor detectors, per-tool dispatch + `observe_plan_tool_result`, plan-awaiting-decision break. S6.20 added per-turn state for the stuck-build-error detector (`check_stuck_error` -- signature streak -> nudge-then-abort, see Key invariants) + the large-overwrite edit_file hint (`note_write_call` / `maybe_hint_large_overwrite`). | `AgentTurnRunner` |
...
| `src/frontends/gui/locus_frame.h/cpp` | Main window. wxAuiManager 3-pane layout, status bar, tab/notebook management, menu wiring. Delegates menu bar to `MenuController`, status-pane composition to `OpsStatusView`, agent-thread event handlers to `AgentEventRouter` (owns it as `router_`; `friend class AgentEventRouter` grants private-state access). | `LocusFrame` |
| `src/frontends/gui/agent_event_router.h/cpp` | Translation layer between agent-thread `wxThreadEvent`s (`EVT_AGENT_TOKEN`, `EVT_AGENT_TOOL_PENDING`, `EVT_AGENT_WATCHDOG_TRIPPED`, ...) and per-tab UI updates. Each handler resolves the tab via `frame_.find_tab_ui(evt.GetId())` and forwards to the matching `ChatPanel` method or updates frame-shared state. Not a `wxEvtHandler` -- owned by LocusFrame; destroyed before Bind teardown so no late event fires into a dangling receiver. | `AgentEventRouter` |
...
| `src/frontends/gui/chat_panel.h/cpp` | Chat UI: wxWebView (HTML/CSS via `build_chat_html()` in [chat_html_template.h](src/frontends/gui/chat_html_template.h)), wxTextCtrl input, footer chips. Streaming via wxTimer + md4c. Owns: mode switcher (Chat / Plan / Execute), in-chat plan bubble (`locus://` scheme intercepted in `on_webview_navigating`), `@`-mention popup, system-prompt bubble (`set_system_prompt_bubble`), per-message delete (`history_to_dom_` map, hover-reveal X), footer chips (`plan_chip_`, `compacted_chip_`, `auto_corrections_chip_`, `commit_btn_`). Commit button is toggled by watchdog tripped/cleared events; click fires `ILocusCore::request_commit_now()`. | `ChatPanel` |
...
| `src/frontends/gui/memory_bank_panel.h/cpp` | Dockable Memory Bank panel. AUI-docked right, default hidden, toggled View > Memory Bank (Ctrl+M). Pure view over `MemoryStore`. Toolbar (search-as-you-type with 200ms debounce, source / tag / pinned-only filters, "Show deleted"), `wxDataViewListCtrl`, edit-in-place detail pane, bulk-op buttons. Live updates via `on_activity_event` on memory_* events + tool_result with `add_memory` / `search_memory` summary. Auto-hides when `capabilities.memory_bank` flips off mid-session. | `MemoryBankPanel` |
...
| `src/frontends/gui/terminal_panel.h/cpp` | Live terminal panel widget. AUI-docked at the frame bottom (default hidden; View > Terminal / Ctrl+`). Pure view over per-tab `TerminalPanelState` -- `set_state(...)` from tab-switch rebuilds the inner `wxNotebook` and replays the canonical `AnsiEvent` log into each STC. Per-tab `wxStyledTextCtrl` with 16-colour palette + bold variants; scrollback capped at 10000 lines. Kill / copy / clear via `KillHandler` callback; bg tabs have a stdin input row forwarding to `StdinHandler`. | `TerminalPanel` |
