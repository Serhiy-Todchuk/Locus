...
| `src/llm/stream_decoder.h` | `IStreamDecoder` interface -- translates one SSE payload into typed events (`on_text` / `on_reasoning` / `on_tool_call_delta` / `on_usage`) via a stack-allocated `StreamDecoderSink`. Decoders may keep per-stream state (XML formats need it). Three optional hooks: `decode()` per chunk, `finish_stream(sink)` once at end-of-stream to flush held-back partial-tag bytes (S4.N), and `reset()` between streams. | `IStreamDecoder`, `StreamDecoderSink` |
