...
**S4.A (Diff-Based Editing) done.** `edit_file` (exact `old_string`/`new_string` with uniqueness check + optional `replace_all`) and `multi_edit_file` (atomic batch, later edits see earlier results) in [src/tools/file_tools.cpp](src/tools/file_tools.cpp). Process-wide `ReadTracker` in [src/tools/shared.h](src/tools/shared.h) forces a prior `read_file` on the same path before edits are accepted — mirrors Claude Code's hallucinated-edit mitigation. Writes go through temp-file + rename for atomicity. System prompt updated to prefer `edit_file` over `write_file`. Approval panel renders edit tools as a red/green unified-diff inside the existing Scintilla pane; `Modify` flips to raw JSON and back. See [roadmap/M4/](roadmap/M4/) for remaining stages.
...
| `src/tools/file_tools.h/cpp` | File CRUD + exact-string-diff edit tools. | `ReadFileTool`, `WriteFileTool`, `EditFileTool`, `MultiEditFileTool`, `CreateFileTool`, `DeleteFileTool` |
| `src/tools/search_tools.h/cpp` | Unified `SearchTool` (mode: text/symbols/semantic/hybrid) — the only search tool registered in the built-in manifest. The four `Search*Tool` classes remain as internal implementations the dispatcher delegates to. | `SearchTool`, `SearchTextTool`, `SearchSymbolsTool`, `SearchSemanticTool`, `SearchHybridTool` |
