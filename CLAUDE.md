...
| `src/tools/tool_registry.h/cpp` | Concrete registry. Schema JSON builder. ToolCall parser. S6.11 added a `bool lazy` param to `build_schema_json(ws, mode, lazy)` that, when true, replaces every tool's `parameters` schema with `{type:"object", properties:{}, additionalProperties:true}` -- except `describe_tool` itself which keeps its real schema (the meta-tool is the entry point for fetching the others). Description text is preserved either way. | `ToolRegistry` |
