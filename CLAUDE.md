...
**M6 -- Connected & Misc -- in progress.** Done: S6.10, S6.11, S6.12, S6.13, S6.15, S6.16, S6.17, S6.18 (partial -- Tasks A-G shipped, remaining items deferred), S6.19, S6.20. Per-stage detail in [roadmap/M6/](roadmap/M6/); cross-cutting design rationale in [ADR-0007](architecture/decisions/0007-context-budget-reshape-lazy-manifest-and-profiles.md).

Agentic-testing follow-ups (offline C++/3D, qwen3.5-9b / qwen3-14b via LM Studio) landed as small fixes: Qwen preset retuned to thinking-mode samplers (0.6 / 0.95); `check_stuck_error` now resets its streak only on real `[exit code: 0]` (previously reset on every clean edit round, so the canonical edit/build thrash never tripped it); UIA harness `type_text` bulk-sets multi-line prompts + `wait_for_agent_idle` reports `awaiting_user`. Two unshipped-but-important findings are captured for triage in [tests/ui_automation/output/agentic_gl_cube/findings.md](tests/ui_automation/output/agentic_gl_cube/findings.md): (1) `lazy_tool_manifest=true` breaks small models (they emit empty-arg tool calls instead of calling `describe_tool`, and the repeated-tool-call cap aborts the turn before the self-heal schema injection lands); (2) code that compiles-but-doesn't-run and hallucinated "I edited X" summaries pass unchecked -- exit code 0 is the only correctness signal.
